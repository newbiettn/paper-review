<?
include_once(APPPATH . 'libraries/phpBibLib/Bibtex.php');
...
class DBibtex extends Bibtex {
	...
	function DBibtex() {
		...
		if($num_args > 1) {
			...
			$res = $this->bibdb->query("SELECT * FROM tbl_bib_meta;");
			...
			if(count($dfiles) > 0 || $changed == true) {
				...
				foreach($afiles as $afname => $afdate) {
					$sql = "INSERT INTO tbl_bib_meta (fname, fdate) VALUES ('".$afname."','".$afdate."');";
					$this->bibdb->query($sql);
				}
				$sql = "COMMIT TRANSACTION;";
				$this->bibdb->query($sql);
	...
	function HasEntry($key) {
		...
		$sql = "SELECT COUNT() FROM tbl_paper WHERE key='".$key."';";
		$res = $this->bibdb->query($sql);
		return $res !== false ? $res->fetchColumn() : false;
	}
	function GetEntry($key) {
		$key = $this->NormalizeKey($key);
		$sql = "SELECT * FROM tbl_paper WHERE key='".$key."';";
		$res = $this->bibdb->query($sql);

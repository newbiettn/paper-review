...
//		$str .=	(isset($entry['publisher']) ? ((isset($entry['booktitle']) || (isset($entry['journal']) || isset($entry['volume'])) ? ', ' : '') . '<span class="bibtex-publisher">' . $entry['publisher'] . '</span>'):'');
	...
	function add_review_button($entry){
        ...
        if (isset($entry['review_fk'])){
            if ($entry["review_fk"] != -1 && $entry["review_fk"] != NULL) {
                $r = "<a target='_blank' href='" . base_url("index.php/paper/manage/open_review/" . $entry["id"] . '/' . $entry["review_fk"]) ."' class='button tiny'>Open Review</a>";
            } else {
                $r = "<a target='_blank' href='" . base_url("index.php/paper/manage/create_view/" . $entry["id"]) ."' class='button tiny alert'>Create a Review</a>";
            }
...
include_once($Path['lib-bibtex'] . 'AbbrvPrinter.php');
include_once($Path['lib-bibtex'] . 'NatbibPrinter.php');
include_once($Path['lib-bibtex'] . 'NumericPrinter.php');
?>